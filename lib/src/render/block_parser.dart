  ...
  List<String?> parseChildLines(BlockParser parser) {
    ...
    while (!parser.isDone) {
      ...
      if (match != null) {
        ...
        var nextMatch = parser.next != null ? pattern.firstMatch(parser.next!) : null;
  ...
  Node parse(BlockParser parser) {
    ...
    while (!parser.isDone) {
      ...
      if (tryMatch(_emptyPattern)) {
        ...
        var line = parser.current.replaceFirst(leadingSpace, ' ' * leadingExpandedTabLength).replaceFirst(indent, '');
        childLines.add(line);
...
class TableSyntax extends BlockSyntax {
  ...
  Element _parseRow(BlockParser parser, List<String?> alignments, String cellType) {
...
class ParagraphSyntax extends BlockSyntax {
  ...
  List<String>? _extractReflinkDefinitions(BlockParser parser, List<String> lines) {
    bool lineStartsReflinkDefinition(int i) => lines[i].startsWith(_reflinkDefinitionStart);
